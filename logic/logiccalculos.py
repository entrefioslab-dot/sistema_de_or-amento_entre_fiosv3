...
def calcular_pacotes_envio_inteligente(itens):
    """
    Calcula o número de pacotes necessários com algoritmo inteligente.
    UNIFICA PEÇAS DE DIFERENTES FORNECEDORES NO MESMO PACOTE.
    Usa a capacidade MÁXIMA disponível entre os produtos para otimizar o empacotamento.
    """
    pacotes = []
    peso_total = 0
    
    # Encontrar a capacidade máxima de pacote disponível
    capacidade_maxima = 10  # padrão
    for item in itens:
    ...
    # Criar lista de todas as peças com suas informações
    todas_as_pecas = []
    for item in itens:
        produto_info = item['produto_info']
    ...
    # Algoritmo de empacotamento inteligente
    ...
    while pecas_restantes:
        ...
        # Preencher pacote até a capacidade máxima
        while pecas_restantes and pacote_atual['quantidade'] < capacidade_maxima:
            ...
            # Adicionar ou atualizar item no pacote
            item_existente = None
            for item_pacote in pacote_atual['itens']:
                if (item_pacote['nome'] == peca['produto_nome'] and 
                    item_pacote['fornecedor'] == peca['fornecedor']):
...
def calcular_resumo_orcamento(itens_selecionados, forma_pagamento):
    """
    Realiza o cálculo completo do orçamento com todas as métricas internas.
    """
    itens_calculados = []
    total_quantidade = 0
    custo_total = 0
    ...
    if total_quantidade >= 100: percentual_desconto = 0.15
    elif total_quantidade >= 50: percentual_desconto = 0.10
    elif total_quantidade >= 20: percentual_desconto = 0.05
    ...
    # Logística com algoritmo inteligente
